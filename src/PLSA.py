...
class PLSATrain(object):
    ...
    def __init__(self, zCount):
        ...
        # Probability of topic given word and doc (normalized)
        self.__pZWDMatrix = None
    ...
    def train(self, docs, precision):
        ...
        self.__pZWDMatrix = np.memmap("pZWDMatrix.dat", dtype='float32',
                                      mode='w+', shape=(self.__kMax, self.__iMax, self.__jMax))
        # Random values to start
        for i in range(self.__kMax):
            self.__pZWDMatrix[i] = np.random.random_sample(
                (1, self.__iMax, self.__jMax))
    ...
    def __updatePZWD(self):
        ...
        for k in range(self.__kMax):
            self.__pZWDMatrix[k] = np.outer(
                tp_pWZMatrix[k], self.__pZDMatrix[k]) / sumMatrix

    def __getSum(self, tp_pWZMatrix):
        sum = np.zeros((self.__iMax, self.__jMax))
        for l in range(self.__kMax):
...
class PLSATest(object):
    ...
    def __init__(self, zCount):
        ...
        self.__iMax = None
        # Max number of individual words
        self.__jMaxNew = None
        # Max number of testing documents
        # Setting up training arrays
        # Probability of topic given word and doc (normalized)
        self.__pZWnewDMatrix = None
        # Setting up testing arrays
        # Probability of topic given word and doc (normalized)
        self.__pWZMatrix = np.load("pWZMatrix.npy")
        self.__pZnewDMatrix = None
        # Probability of topic given a doc
